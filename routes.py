...
def compras(u_id):
    db = Database()
    cur = db.cursor_dict()
    cur.execute("""SELECT p.p_nombre, cv.cv_cant ,cv.cv_cant * p.p_precio FROM producto p, compravirtual cv, presupuesto pr WHERE cv.u_id=%s and cv.p_id=p.p_id and pr.pre_id is not null""", (u_id,))
    comprasv = cur.fetchall()
    cur.execute("""SELECT SUM(cv.cv_cant * p.p_precio) FROM producto p, compravirtual cv, presupuesto pr WHERE cv.u_id=%s and cv.p_id=p.p_id and pr.pre_id is not null""", (u_id,))
    total = cur.fetchone()
    db.cerrar()
